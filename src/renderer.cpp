...
namespace big
{
	...
	void renderer::imgui_init()
	{
		...
		if (!load_from_memory(bnw_header, _ARRAYSIZE(bnw_header), m_d3d_device, &m_header, &m_header_size.x, &m_header_size.y))
			LOG(WARNING) << "Unable to load image header";

		if (!load_from_memory(toggle_texture, _ARRAYSIZE(toggle_texture), m_d3d_device, &m_toggle, &m_toggle_size.x, &m_toggle_size.y))
    ...
    bool renderer::load_from_file(const char *filename, ID3D11Device *d3dDevice, ID3D11ShaderResourceView **out_srv, int *out_width, int *out_height)
    {
	...
	bool renderer::load_from_memory(const unsigned char* buffer, int buffer_size, ID3D11Device* d3dDevice, ID3D11ShaderResourceView** out_srv, int* out_width, int* out_height)
	{
